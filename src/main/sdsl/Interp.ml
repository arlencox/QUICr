  ...
  let interpret p =
    ...
    let rec interpret state = function
      ...
      | Rename l ->
        ...
        let state = D.rename_symbols (QUICr.Rename.of_assoc_list l) state in
        if !print_step then print_state renv state;
        state
      ...
      | For(v,e,t) ->
        ...
        let rec loop i invstate =
          ...
          let body_end =
            ...
            D.forget [vid] |>
            D.rename_symbols (QUICr.Rename.singleton vid' vid) |>
            (fun st -> interpret st t) |>
            D.constrain (L.Eq(L.Var visit_id', L.DisjUnion(L.Var visit_id, L.Sing vid))) |>
            D.forget [visit_id] |>
            D.rename_symbols (QUICr.Rename.singleton visit_id' visit_id)
          in
      ...
      | Assign (lhs, rhs) ->
        ...
        let state = if do_rename then
            ...
            state |>
            D.forget [oldid] |>
            D.rename_symbols (QUICr.Rename.singleton lid oldid) (*[lid,oldid]*) (*(fun id -> if id = lid then oldid else id)*)
          else state in
        if !print_step then print_state renv state;
        state
      | Choose (lhs, rhs) ->
