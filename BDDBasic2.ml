module Make
    (S : Interface.Sym)
    (B : Interface.NonRel with type sym = S.t)
...
struct
  ...
  let reduce_b_to_s a =
    SMap.fold (fun sym (b,bv) a ->
        match B.constant b with
        | None -> a
        | Some c ->
          let (a,id) = add_constant c bv a in
          let id = Cudd.Bdd.ithvar a.ctx id in
          let sid = SMap.find sym a.s2i in
          let sid = Cudd.Bdd.ithvar a.ctx sid in
          let bdd' = Cudd.Bdd.eq sid id in
          { a with bdd = Cudd.Bdd.dand a.bdd bdd' }
      ) a.base a
  ...
  let merge base_op va abase vb bbase vc cbase =
    let res = try
      let (base1,bv1) = SMap.find va abase in
      try
        let (base2,_bv2) = SMap.find vb bbase in
        Some (base_op base1 base2, bv1)
      with Not_found->
        Some (base_op base1 B.top, bv1)
    with Not_found ->
      try 
        let (base2,bv2) = SMap.find vb bbase in
        Some (base_op B.top base2, bv2)
      with Not_found ->
        None
    in
    match res with
    | None ->
      SMap.remove vc cbase
    | Some (base, _) when B.is_top base ->
      SMap.remove vc cbase
    | Some b ->
      SMap.add vc b cbase


  let ul_bound op mapping a b =
    let bdd_op, base_op = match op with
    | Join -> (Cudd.Bdd.dor, B.join)
    | Widening -> (Cudd.Bdd.dor, B.widening)
    | Meet -> (Cudd.Bdd.dand, B.meet)
    in
    let (a,b,c,imapping) = constant_mapping (op = Widening) a b in

    (* construct integer mapping for symbols and do base operation *)
    let (imapping,c) = List.fold_left (fun (imapping,c) (va,vb,vc) ->
        let ia = SMap.find va a.s2i in
        let ib = SMap.find vb b.s2i in
        let base = merge base_op va a.base vb b.base vc c.base in
        ...
        let c = {c with s2i; i2cs; free; base} in
        ((ia,ib,ic)::imapping, c)
    ...
    let bddc = bdd_op bdda bddb in
    {c with bdd = bddc}

  let meet = ul_bound Meet

  let join = ul_bound Join

  let widening = ul_bound Widening
  ...
  let get_value va a =
    try
      fst (SMap.find va a.base)
    with Not_found ->
      B.top

  let le mapping a b =
    let a = reduce_b_to_s a in
    let b = reduce_b_to_s b in
    (* check base domain first *)
    if List.for_all (fun (va,vb) ->
        let val_a = get_value va a in
        let val_b = get_value va b in
        B.le val_a val_b
      ) mapping then begin
  ...
  let top ctx syms =
    let (free,s2i,i2cs,base) = List.fold_left (fun (free,s2i,i2cs,base) s ->
        let (free, i) = Fresh.fresh free in
        let s2i = SMap.add s i s2i in
        let i2cs = IMap.add i (Symbol s) i2cs in
        (free,s2i,i2cs,base)
      ) (Fresh.empty,SMap.empty,IMap.empty,SMap.empty) syms in
    {
      ctx = ctx;
